...
        let stream = this.streams.get(cameraKey);
        if (!stream || stream.state === 'ERROR') {
            stream = this._createCameraStream(cameraKey, cameraDetails);
            this.streams.set(cameraKey, stream);
        }
...
        // If stream is IDLE, start a new connection
        if (currentStream.state === 'IDLE') {
...
                this._cleanupStream(stream, `Non-200 status code: ${response.statusCode}`);
                return;
            }
            
...
        if (stream.clients.size > 0 || stream.cleanupTimeout) return;
    
        console.log(`[STREAM] No clients for ${stream.key}. Scheduling cleanup in ${this.idleTimeout / 1000}s.`);
    
        stream.cleanupTimeout = setTimeout(() => {
            // Check again in case a client reconnected in the meantime
            if (stream.clients.size === 0) {
                this._cleanupStream(stream, `Idle timeout reached for ${stream.key}.`);
            }
        }, this.idleTimeout);
    }
    
...
        
        // Remove the stream completely, it will be recreated on next connect
        this.streams.delete(stream.key);
    }

    _broadcastError(stream, message) {
        console.log(`[STREAM] Broadcasting error to ${stream.clients.size} clients for ${stream.key}: ${message}`);
        for (const clientRes of stream.clients) {
